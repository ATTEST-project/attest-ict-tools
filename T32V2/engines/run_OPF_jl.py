...
def run_jl(folder):
    ...
    #WP6
    #must distinguish data path (in folder variable) from sources path
    #Main.include(folder+'main.jl')
    #+++++ current dir: C:\opt\T32\ATTEST_Tool3.2
    #+++++ folder: C:\opt\T32\tmp\SCOPF_R5\
    Main.data_path = folder
    juliascript_path = str(os.sep.join([os.path.realpath(os.path.dirname(__file__)), 'SCOPF_R5', 'main.jl']))
    print('+++++ juliascript_path: ' + juliascript_path)
    try:
        Main.include(juliascript_path)
...
def run_SCACOPF_jl(input_dir,mpc, cont_list, penalty_cost , sbase = 100):
    ...
    #WP6 debug
    print('+++++++++ julia output path: ' + str(folder+'data_preparation\\export_WP3.json'))
