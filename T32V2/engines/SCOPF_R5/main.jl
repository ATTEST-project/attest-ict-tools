...
#WP6 note: data_path coming from pyjulia, set in run_OPF_jl.py
#filename = JSON.parsefile("data_preparation\\import_WP3.json")["filename"] # full path
println(string("***** data_path: ", data_path))
import_wp3_path = string(data_path, "data_preparation\\import_WP3.json")
println(string("***** import_wp3_path: ", import_wp3_path))
filename = JSON.parsefile(import_wp3_path)["filename"] # full path
#adjusts the path separator. TBD make it better
filename = replace(string(data_path, filename), "/" => "\\")
println(string("***** filename: ", filename))

##
# WP6
#filename_scenario= "input_data/scenario_gen.ods"
filename_scenario= string(data_path, "input_data/scenario_gen.ods")
