...
defaultEVFileName = 'EV-PV-Storage_Data_for_Simulations.xlsx'
...
def run_screening(input_dir, output_dir,ods_file_name, xlsx_file_name, country, test_case, peak_hour,no_year,add_load_data,add_load_data_case_name): 
    ...
    run_main_screening(input_dir, output_dir, ods_file_name, xlsx_file_name, country, test_case, peak_hour,no_year,add_load_data,add_load_data_case_name,defaultEVFileName)
...
def run_all(input_dir, output_dir, ods_file_name, xlsx_file_name, country, test_case, peak_hour, no_year, run_both, add_load_data, add_load_data_case_name):
    ...
    print(" --------- Running the investment model --------- ")
    run_main_investment(input_dir, output_dir, ods_file_name, xlsx_file_name, country, test_case, peak_hour, no_year, run_all, add_load_data, add_load_data_case_name,defaultEVFileName) 
...
@cli.command('run_wp6')
@click.option('--input_file', default='', help='input json file')
def pyensys_ATTEST_Distribution_wp6(**kwargs):
    ''' Call ATTEST's transmission network planning tool - wp6 version: inputs from a json file'''
    input_file = kwargs.pop('input_file')
    with open(input_file) as json_file:
        data1 = json.load(json_file)
    data = data1["parameters"]

    if (data["model"] == "screening" or data["model"] == "all"):
        print(" --------- Running the screening model --------- ")
        run_main_screening(data["input_dir"],
                           data["output_dir"],
                           data["ods_file_name"],
                           data["xlsx_file_name"],
                           data["country"],
                           ...
                           data["peak_hour"],
                           data["no_year"],
                           data["use_load_data_update"],
                           data["add_load_data_case_name"],
                           data["EV_data_file_name"])
