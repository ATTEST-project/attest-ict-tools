...
def Screenning_clusters(gen_status, line_status, test_case, multiplier,
                       ...
                       Max_clusters, use_load_data_update,add_load_data_case_name, output_directory_path, EV_data_file_path):
    ...
    multiplier_bus = mult_for_bus(busMult_input, multiplier, flex, mpc, output_directory_path)
    ...
    interv_dict, interv_clust = \
        main_screening(mpc, gen_status, line_status, multiplier_bus,
                       ...
                       cont_list, Q_load_correction, use_load_data_update, add_load_data_case_name, output_directory_path, EV_data_file_path)
    ...
    with open(os.path.join(output_directory_path, file_name+".json"), 'w') as fp:
...
def save_in_jsonW_2(solution, output_dir, NoLines, case_name, yrs=[], scen=[]):
    ...
    print('\nResults have been saved to ' + os.path.abspath(output_dir))


def attest_invest_proc(output_dir, test_case, attest_inputs_path, line_capacities, trs_capacities, cont_list, cluster,
                   line_length, scenarios, oversize, line_costs, trs_costs, growth, DSR, Max_clusters, use_load_data_update, add_load_data_case_name, EV_data_file_path):
    '''Call ATTEST's distribution network planning tool '''
    # test_case = kwargs.pop('case') # to give an absolute path
    ci_catalogue = [line_capacities, trs_capacities]
    ...
    output_directory_path = os.path.dirname(output_dir)

    # ## dump all the parameters values
    print('++ output_dir:' + output_dir)
    print('++ test_case:' + test_case)
    print('++ attest_inputs_path:' + attest_inputs_path)
    print('++ ci_catalogue:' + str(ci_catalogue))
    print('++ cont_list:' + str(cont_list))
    print('++ cluster:' + str(cluster))
    print('++ line_length:' + str(line_length))
    print('++ scenarios:' + str(scenarios))
    print('++ oversize:' + str(oversize))
    print('++ ci_cost:' + str(ci_cost))
    print('++ growth:' + str(growth))
    print('++ DSR:' + str(DSR))
    print('++ Max_clusters:' + str(Max_clusters))
    print('++ use_load_data_update:' + str(use_load_data_update))
    print('++ add_load_data_case_name:' + add_load_data_case_name)
    print('++ EV_data_file_path:' + EV_data_file_path)
    ...
    if final_interv_clust == []:
        ...
        with open(attest_inputs_path, 'w', encoding='utf-8') as f:
        ...
        if use_clusters_verification == True: # use the clusters verification algorithm
            ...
            solution, no_feasible_plan = clust_verification_function(test_case,use_load_data_update,add_load_data_case_name,mpc['branch']['TAP'], output_directory_path, EV_data_file_path)
            ...
            solution, info = main_access_function(file_path=attest_inputs_path)
...
def attest_invest(kwargs):
    ...
    test_case = os.path.join(Base_Path, 'tests', 'matpower', kwargs.pop('case'))
    line_capacities = kwargs2ListF(kwargs, 'line_capacities')
    trs_capacities = kwargs2ListF(kwargs, 'trs_capacities')
    ...
    attest_inputs_path = os.path.join(Base_Path, 'tests', 'json', 'ATTEST_Inputs.json')

    EV_data_file_path = os.path.join(Base_Path, "tests", "EV-PV-Storage_Data_for_Simulations.xlsx")

    attest_invest_proc(output_dir, test_case, attest_inputs_path, line_capacities, trs_capacities, cont_list,
                       cluster, line_length, scenarios, oversize, line_costs, trs_costs, growth, DSR, Max_clusters,
                       use_load_data_update, add_load_data_case_name, EV_data_file_path)


def attest_invest_wp6(kwargs):
    ...
    attest_invest_proc(data["output_path"], data["case_path"], data["attest_inputs_path"], data["line_capacities"],
                       data["TRS_capacities"], data["cont_list"],
                       cluster, data["line_length"], data["scenarios"], data["oversize"],
                       data["line_costs"], data["TRS_costs"], data["growth"], data["DSR"], data["Max_clusters"],
                       data["use_load_data_update"], data["add_load_data_case_name"], data["EV_data_file_path"])
