...
def clust_verification_function(case_path,use_load_data_update,add_load_data_case_name,line_tap_vector, output_directory_path, EV_data_file_path): # Note: multipliers and clusters are saved in json and then read whitin this function
    ...
    # # Read clusters data:
    with open(os.path.join(output_directory_path, "screen_result_final_interv_clust.json"), 'r') as f:
        screen_result_final_interv_clust = json.load(f)
    # # Read load multipliers:   
    with open(os.path.join(output_directory_path, "mult_bus_export.json"), 'r') as f:
