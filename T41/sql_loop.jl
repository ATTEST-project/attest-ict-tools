...
str_bv,fl_bv,oltc_bv,oltc_aux_bv,ratio_sql,p_inj_nl,q_inj_nl,avg_gen_curt,avg_str_chr,avg_str_dch,avg_str_soc,avg_fl_od,avg_fl_ud,avg_tratio,line_alpha,line_eta,line_beta,dg_pf_value) = opf_results_variables(nTP,nSc,nBus,nLines,nNcurt_gen,nCurt_gen,nFl,nStr_active,nTrsf,nTrsf_s1,n_tap)
    ...
    while (vol_viol_max > vol_viol_tol || crnt_viol_max > crnt_viol_tol) && lin_itr <=lin_itr_max && opf_term_status == 0
        println(" ")
        display("Linear Iteration: $lin_itr")
        println(" ")


        (vm_ac,va_ac,vm_plr,va_plr,vm_min,vm_max,gen_Ncurt_P,gen_Ncurt_Q,gen_Curt_P_sc,gen_Curt_P_lin,total_p_curt,fl_inc_sc,fl_dec_sc,fl_inc_nd_lin,fl_dec_nd_lin,p_ch_nd_lin,p_dis_nd_lin,soc_nd,p_strg_nd,trsf_ratio,gen_Curt_Q_lin,
        br_crnt_pu,br_crnt_si,br_pwr_ij_pu,br_pwr_ij_SI,br_crnt_ij_pu_ex,br_crnt_ij_SI_ex,br_pwr_ij_pu_ex,br_pwr_ij_SI_ex,app_err,p_inj,q_inj,p_inj_nl,q_inj_nl,p_imp_td,q_imp_td,str_bv,fl_bv,oltc_bv,oltc_aux_bv,ratio_sql,
        v_mag_int,v_ang_int,max_err_per_bus,norm_s_err_max_power,p_err_max_power,q_err_max_power,avg_err_per_bus,norm_s_err_avg,p_err_avg,q_err_avg,rel_max_error,rel_avg_error,sql_itr,sql_obj,sql_time,
        avg_gen_curt,avg_str_chr,avg_str_dch,avg_str_soc,avg_fl_od,avg_fl_ud,avg_tratio,oltc_ratio,infsb_model,term_status) = sql_opf_model(vol_nodes_mag_pwr_flow,vol_nodes_theta_pf,nSc,nTP,nBus,nNcurt_gen,nCurt_gen,nTrsf,nStr_active,nFl,nd_fl,flex_apc,flex_oltc,flex_adpf,flex_str,flex_fl,str_bin,fl_bin,rdata_buses,nLines,trsf_fict_nodes,tap_ratio_range,
        ...
        yii_sh,i_curt_gens,sbase,dg_pf,iStr_active,yij_line,dLines,i_ncurt_gens,nw_lines,bus_data_gsheet,pg_min,qg_min,qg_max,pgen_tol,qgen_tol,p_tol,q_tol,rcvr_branch,rcvr_inj,rcvr_tso_dso,Ibase,idx_slack_bus_pf,solver_ipopt,solver_cbc,solver_bonmin,sql_itr,sql_itr_max,s_inj_error_rel,s_error_tol,angle_lb,angle_ub,cycleList,idx_slack_from,idx_slack_to,
        num_slack_cnctd_nodes,slack_cnctd_nodes,slack_nd,stoch_model,tratio_init,rdata_storage,load_theta,nd_Str_active,max_err_per_bus,norm_s_err_max_power,p_err_max_power,q_err_max_power,avg_err_per_bus,norm_s_err_avg,p_err_avg,q_err_avg,rel_max_error,rel_avg_error,sql_obj,sql_time,lin_itr,v_initial_pf,min_vol_limit,max_vol_limit,max_crnt_limit,oltc_ratio,
        vm_ac,va_ac,vm_plr,va_plr,vm_min,vm_max,gen_Ncurt_P,gen_Ncurt_Q,gen_Curt_P_sc,gen_Curt_P_gen,total_p_curt,fl_inc_sc,fl_dec_sc,fl_inc_nd,fl_dec_nd,p_ch_nd,p_dis_nd,soc_nd,p_strg_nd,
        trsf_ratio,dgpf,gen_Curt_Q,br_crnt_pu,br_crnt_si,br_pwr_ij_pu,br_pwr_ij_SI,br_crnt_ij_pu_ex,br_crnt_ij_SI_ex,br_pwr_ij_pu_ex,br_pwr_ij_SI_ex,app_err,p_inj,q_inj,p_imp_td,q_imp_td,
        str_bv,fl_bv,oltc_bv,oltc_aux_bv,ratio_sql,p_inj_nl,q_inj_nl,avg_gen_curt,avg_str_chr,avg_str_dch,avg_str_soc,avg_fl_od,avg_fl_ud,avg_tratio,line_alpha,line_eta,line_beta,dg_pf_value,v_mag_int_prv_itr,v_ang_int_prv_itr,term_status,solver_cplex,int_tol,opt_tol,num_thread,rdata_trsf,nTrsf_s1,nw_trsf,n_tap)
...
println("")
        ...
        (vol_nodes_mag_pwr_flow_lin,vol_nodes_theta_pf_lin,vol_rect_pf_ip,p_slack_pf_ip,q_slack_pf_ip,v_rect_check) = ac_power_flow_opf(nBus,nLines,nNcurt_gen,rdata_buses,nSc_pf,nTP_pf,prob_scs_pf,time_step,stoch_model,p_curt_opf_lin,
        p_od_opf_lin,p_ud_opf_lin,p_strg_opf_lin,q_dg_opf_lin,nw_buses,nw_lines,rdata_loads,bus_data_lsheet,bus_data_Ssheet,node_data,nd_fl,nd_curt_gen,nd_ncurt_gen,p_load,q_load,idx_Gs_lsheet,idx_Bs_lsheet,yii_sh,i_curt_gens,sbase,flex_oltc,vbase,Ibase,
        pg_min_pf,pg_max_pf,qg_min_pf,qg_max_pf,yij_line,dLines,i_ncurt_gens,error_msg,pgen_tol,qgen_tol,trsf_ratio_opf_lin,vol_cstr_tol)

        (br_crnt_pu_pf_lin,br_crnt_si_pf_lin,br_pwr_pu_pf_lin,br_pwr_si_pf_lin) = recovery_branch_current_pf(nSc,nTP,nLines,nw_lines_pf,vol_rect_pf_ip,yij_line,Ibase,sbase,ybase,vbase,oltc_ratio,dLines)

        (vol_viol,crnt_viol,max_vol_viol,max_crnt_viol,viol_nodes,nVol_viol_lin,avg_vol_viol_lin,above_avg_vol_viol_lin,below_avg_vol_viol_lin,
        nCrnt_viol_lin,avg_crnt_viol_lin,above_avg_crnt_viol_lin,below_avg_crnt_viol_lin,min_vol_limit,max_vol_limit,max_crnt_limit,vol_viol_cn_milp) =
        constraint_violation(nBus,nw_buses,nLines,nw_lines,nTP,nSc,vol_nodes_mag_pwr_flow_lin,br_crnt_si_pf_lin,I_rat,lin_itr,
        min_vol_limit,max_vol_limit,max_crnt_limit,vol_viol_tol,crnt_viol_tol,vol_cnst_fctr,crnt_cnst_fctr,lin_itr_max)
