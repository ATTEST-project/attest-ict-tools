...
elseif solver_cplex == 1
    # Original code, requires IBM CPLEX distribution
    # acopf = direct_model(CPLEX.Optimizer())
    # set_optimizer_attribute(acopf, "CPX_PARAM_EPINT", int_tol)
    # set_optimizer_attribute(acopf, "CPX_PARAM_EPOPT", opt_tol)
    # set_optimizer_attribute(acopf, "CPX_PARAM_BARALG", 3)
    # set_optimizer_attribute(acopf, "CPX_PARAM_SOLUTIONTYPE", 2)
    # set_optimizer_attribute(acopf, "CPX_PARAM_DATACHECK", CPX_ON)
    # set_optimizer_attribute(acopf, "CPX_PARAM_THREADS", num_thread)

    # Changes to make the tool work using a CPLEX bundled with AMPL-community
    # instead of the regular IBM CPLEX Distribution.
    # (please change the path to the optimizer in the line below, if cplex.exe is not in c:/ATTEST/ampl)
    acopf = Model(() -> AmplNLWriter.Optimizer("C:/ATTEST/ampl/cplex.exe"))
    set_optimizer_attribute(acopf, "integrality", int_tol)
    set_optimizer_attribute(acopf, "optimality", opt_tol)
    set_optimizer_attribute(acopf, "baralg", 3)
    set_optimizer_attribute(acopf, "solutiontype", 2)
    set_optimizer_attribute(acopf, "datacheck", 1)
    set_optimizer_attribute(acopf, "threads", 1)
end
...
################################################################################
function opf_model_solution(term_status,v_mag,v_ang,Pg,Qg,Pg_curt,p_ch,p_dis,soc,p_strg,p_fl_inc,p_fl_dec,tratio,bin_ch,bin_fl,bin_tap,aux_tap,flex_str,flex_fl,flex_oltc,flex_adpf,nNcurt_gen,nCurt_gen,nFl,nTrsf,nLines,nw_lines,node_data,rdata_buses,yij_line,pg_max,pgen_tol,qgen_tol,p_tol,q_tol,prob_scs,nTP,nSc,rcvr_branch,rcvr_inj,rcvr_tso_dso,i_curt_gens,sbase,v_mag_int,v_ang_int,Ibase,dLines,idx_slack_bus_pf,trsf_fict_nodes,vm_ac,va_ac,vm_plr,va_plr,vm_min,vm_max,gen_Ncurt_P,gen_Ncurt_Q,gen_Curt_P_sc,gen_Curt_P_gen,total_p_curt,fl_inc_sc,fl_dec_sc,fl_inc_nd,fl_dec_nd,p_ch_nd,p_dis_nd,soc_nd,p_strg_nd,trsf_ratio,dgpf,gen_Curt_Q,br_crnt_pu,br_crnt_si,br_pwr_ij_pu,br_pwr_ij_SI,br_crnt_ij_pu_ex,br_crnt_ij_SI_ex,br_pwr_ij_pu_ex,br_pwr_ij_SI_ex,app_err,p_inj,q_inj,p_imp_td,q_imp_td,str_bv,fl_bv,oltc_bv,oltc_aux_bv,ratio_sql,p_inj_nl,q_inj_nl,avg_gen_curt,avg_str_chr,avg_str_dch,avg_str_soc,avg_fl_od,avg_fl_ud,avg_tratio,oltc_ratio,dg_pf_value,dg_pf,Qg_curt,rdata_trsf,nTrsf_s1,nw_trsf,n_tap)
...
return vm_ac,va_ac,vm_plr,va_plr,vm_min,vm_max,gen_Ncurt_P,gen_Ncurt_Q,gen_Curt_P_sc,gen_Curt_P_gen,total_p_curt,fl_inc_sc,fl_dec_sc,fl_inc_nd,fl_dec_nd,p_ch_nd,p_dis_nd,soc_nd,p_strg_nd,trsf_ratio,gen_Curt_Q,
br_crnt_pu,br_crnt_si,br_pwr_ij_pu,br_pwr_ij_SI,br_crnt_ij_pu_ex,br_crnt_ij_SI_ex,br_pwr_ij_pu_ex,br_pwr_ij_SI_ex,app_err,p_inj,q_inj,p_imp_td,q_imp_td,str_bv,fl_bv,oltc_bv,oltc_aux_bv,ratio_sql,p_inj_nl,q_inj_nl,
avg_gen_curt,avg_str_chr,avg_str_dch,avg_str_soc,avg_fl_od,avg_fl_ud,avg_tratio,oltc_ratio_tmp
end
