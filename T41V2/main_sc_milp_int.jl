...
using Statistics
using Printf
## WP6 orig code replaced for integration purposes: using Alert
...
CasesList =    Dict([
    ...
    ("UK_Dx_01_2050_Distribution_Network_Urban_UK_W_Without_1_7_", "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/UK DX 01/Obsolete_but_uploaded/UK_Dx_01_2050_Distribution_Network_Urban_UK_W_Without_1_7.ods"),
    ...
    ("HR_Dx_01_2030_W_Bd_brown_" , "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/HR DX 01/Obsolete but uploaded/BROWN/HR_Dx_01_2030_W_Bd_brown_WithFlex_BROWN.ods" ),
    ("HR_Dx_01_2040_Su_Bd_brown_", "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/HR DX 01/Obsolete but uploaded/BROWN/HR_Dx_01_2040_Su_Bd_brown_WithFlex_BROWN.ods"),
    ...
    ("HR_Dx_01_2050_W_Bd_red_"   , "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/HR DX 01/Obsolete but uploaded/RED/HR_Dx_01_2050_W_Bd_red_WithFlex_RED.ods"       ),
    ...
    ("ES_DX_03_BaseCase"  , "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/ES DX 03/BaseCase.ods"          ),
    ("ES_Dx_03_2030_S_Bd_", "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/ES DX 03/ES_Dx_03_2030_S_Bd.ods"),
    ("ES_Dx_03_2030_W_Bd_", "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/ES DX 03/ES_Dx_03_2030_W_Bd.ods"),
    ("ES_Dx_03_2040_S_Bd_", "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/ES DX 03/ES_Dx_03_2040_S_Bd.ods"),
    ("ES_Dx_03_2040_W_Bd_", "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/ES DX 03/ES_Dx_03_2040_W_Bd.ods"),
    ...
    ("ES_Dx_03_2050_W_Bd_", "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/ES DX 03/ES_Dx_03_2050_W_Bd.ods"),

    ("PT_DX_01_BaseCase"   , "input_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/PT DX 01/OlderButUploadedToCloud/ReinforcedLines/BaseCase.ods"           ),
...
# ==============================================================================
## WP6 orig code replaced for integration purposes:if  true # specify which case to run, by specifying the case's name
## WP6 orig code replaced for integration purposes:    TheKey      = "PT_Dx_01_2040_Su_Bd_" # PT_Dx_01_2040_W_Bd_Reinforced # PT_Dx_01_2040_Su_Bd_Reinforced
## WP6 orig code replaced for integration purposes:    i           = findfirst(isequal(TheKey),CaseNames)
## WP6 orig code replaced for integration purposes:    #### i = findall(x -> occursin(TheKey, x),CaseNames) # also works
## WP6 orig code replaced for integration purposes:    CustomCaseName = "Testing_INFSBL_ExitFlag" # Edit the case name. comment out if not applicable
## WP6 orig code replaced for integration purposes:
## WP6 orig code replaced for integration purposes:else    # specify which case to run, by specifying the case's number
## WP6 orig code replaced for integration purposes:    i         = 2
## WP6 orig code replaced for integration purposes:    TheKey    = CaseNames[i]
## WP6 orig code replaced for integration purposes:end
CustomCaseName = ""
TheKey = get(parameters, "case_name", "")
# ==============================================================================

## WP6 orig code replaced for integration purposes: OutLog      = "output_data/March2023_LoadDataFromAttestCloud/EditedInputFiles/OutLog.xlsx"
OutLog = get(parameters, "outlog_file", "")
mkpath(dirname(OutLog)) ## WP6 create intermediate dirs if they do not exist
...
## WP6 orig code replaced for integration purposes: filename_mat    = CasesList[TheKey]
## WP6 orig code replaced for integration purposes: # filename_mat    = replace(filename_mat  , ".ods"      =>"_WithFlex.ods") # added temporarily. should be disabled.
## WP6 orig code replaced for integration purposes: filename_addt   = replace(filename_mat  , ".ods"      =>"_flex.ods")
## WP6 orig code replaced for integration purposes: file_op         = replace(filename_mat  , "input_data"=>"output_data")
## WP6 orig code replaced for integration purposes: file_op         = replace(file_op       , ".ods"      =>string(Flex_Case,"_output.xlsx"))
filename_mat =  get(parameters, "network_file", "")
filename_addt =  get(parameters, "auxiliary_file", "")
file_op =  get(parameters, "output_file", "")
i = 0 #WP6 this variables is meaningless: declared to not break the CaseLog function signature
mkpath(dirname(file_op)) ## WP6 create intermediate dirs if they do not exist

## WP6 orig code replaced for integration purposes: @printf("Case #%3i/%i: %s\n", i, N_Cases, TheKey)
@printf("Case: %s\n", TheKey)
...
## WP6 orig code replaced for integration purposes: filename = "input_data/scenario_gen.ods"                                         # Reading Scenario files generated by Scenario Generation Tool Box
...
## WP6 START -------------------------------------------------------------
## WP6: code to overwrite some constants, from parameters passed to the tool
...
## WP6: code from constants.jl where scenario is originally set to 1, in case its value is overwritten by the parameter scenario
if scenario == 1
...
## WP6: params values directly set from parameters passed to the tool
## WP6 orig code replaced for integration purposes: if  occursin("WOFlex", CaseLog[2])
## WP6 orig code replaced for integration purposes:     println("Case withOUT Flexibility. Skipping")
## WP6 orig code replaced for integration purposes:     # continue
## WP6 orig code replaced for integration purposes:     flex_fl   = 0
## WP6 orig code replaced for integration purposes:     fl_bin    = 0
## WP6 orig code replaced for integration purposes:     flex_str  = 0
## WP6 orig code replaced for integration purposes:     str_bin   = 0
## WP6 orig code replaced for integration purposes: 
## WP6 orig code replaced for integration purposes:     flex_oltc = 0    # On-load tap changing transfomer
## WP6 orig code replaced for integration purposes:     oltc_bin  = 0    # Binary variables for the modeling of OLTC transformer
## WP6 orig code replaced for integration purposes: 
## WP6 orig code replaced for integration purposes:     # There are others which i'm not changing. such as APC.
## WP6 orig code replaced for integration purposes:     println("case has no flexibility. Storage and Flexible_Load variables were set to zero")
## WP6 orig code replaced for integration purposes: else
## WP6 orig code replaced for integration purposes:     flex_fl   = 1
## WP6 orig code replaced for integration purposes:     fl_bin    = 1
## WP6 orig code replaced for integration purposes:     flex_str  = 1
## WP6 orig code replaced for integration purposes:     str_bin   = 1
## WP6 orig code replaced for integration purposes: end

## WP6 START ----------------------------------
## WP6 dump values that were set from parameters

println("");
println("WP6 T41V2 (release 05/07/2023)");
println("WP6 dumping parameters values (parameter name/ julia variable name)");
println("WP6 case_name/TheKey: $TheKey");
println("WP6 filename_mat/network_file: $filename_mat");
println("WP6 filename_addt/auxiliary_file: $filename_addt");
println("WP6 file_op/output_file: $file_op");
println("WP6 filename/scenario_file: $filename");
println("WP6 OutLog/outlog_file: $OutLog");

println("WP6 nTP/ntp: $nTP");
println("WP6 scenario/scenario: $scenario");

println("WP6 flex_apc: $flex_apc");
println("WP6 flex_oltc: $flex_oltc");
println("WP6 oltc_bin: $oltc_bin");
println("WP6 flex_adpf: $flex_adpf");
println("WP6 flex_fl: $flex_fl");
println("WP6 fl_bin: $fl_bin");
println("WP6 flex_str: $flex_str");
println("WP6 str_bin: $str_bin");
...
## WP6 orig code replaced for integration purposes:     alert("JULIA: NO violations. This case is a waste of time")
    ...
    LogThis(CaseLog=CaseLog, Append=false)
 ## WP6 orig code replaced for integration purposes:    alert("JULIA: $(sum(N_violations)) Violations FOUND. Optimization will commence")
...
CaseLog[9] = MILP_term_status  # 6:"Init_PF", 7:"Viols_Exist", 8:"Opt_Commenced", 9:"Opt_Found"
LogThis(CaseLog=CaseLog, Append=false)
## WP6 orig code replaced for integration purposes: alert(string("Finished optimization. Results exported to Excel: ", TheKey))
