...
# ==============================================================================
if  true # specify which case to run, by specifying the case's name
    TheKey      = "PT_Dx_01_2040_Su_Bd_" # PT_Dx_01_2040_W_Bd_Reinforced # PT_Dx_01_2040_Su_Bd_Reinforced
    i           = findfirst(isequal(TheKey),CaseNames)
    #### i = findall(x -> occursin(TheKey, x),CaseNames) # also works
    CustomCaseName = "Testing_INFSBL_ExitFlag" # Edit the case name. comment out if not applicable
...
# filename_mat    = replace(filename_mat  , ".ods"      =>"_WithFlex.ods") # added temporarily. should be disabled.
filename_addt   = replace(filename_mat  , ".ods"      =>"_flex.ods")
file_op         = replace(filename_mat  , "input_data"=>"output_data")
file_op         = replace(file_op       , ".ods"      =>string(Flex_Case,"_output.xlsx"))
