...
if endswith(filename, ".m")
    println("Input network in MATPOWER (.m) format: $filename")
	println("Convert MATPOWER to ODS")
	mfile_name = filename
	filename = replace(filename, ".m" => ".ods")
	include("data_preparation\\data_converter.jl")
else
    println("Input network in ODS format: $filename")
	println("No conversion from MATPOWER (.m) needed.")
end
